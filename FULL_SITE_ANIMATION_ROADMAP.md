...
首批实施从 05、07、10、11 开始。**已完成并发布**：05 · RadixAttention、07 · Chunked Prefill、11 · Overlap Scheduler 的预测闭环实验与中文旁白 Manim 动画；10 · Speculative Decoding 的预测闭环实验已完成，原理动画仍在本波次待制作。每个章节按“先更新实验状态机与代码锚点，再制作音频先行的 Manim 动画，再嵌入章节”的顺序完成。每完成一章，都运行独立实验语法检查；每完成一波，运行 `npm run check`、在浏览器验证正确预测与至少一个反事实分支，并更新 `INTEGRATION_NOTES.md`。
